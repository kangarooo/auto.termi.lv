...
var DoubleSlider = new Class({
  	...
  	this.setOptions(options);
		...
		switch (this.options.mode)
		{
			case 'vertical':
				...
				this.property = 'top';
				offset = 'offsetHeight';
				this.offsetSet = 'height';
				break;
			case 'horizontal':
				this.axis = ['x', 'y'];
				this.property = 'left';
				offset = 'offsetWidth';
				this.offsetSet = 'width';
		}
		
		// Knöpfe herausfiltern
		var knobs = this.options.knobs === false ? (this.element.getElements('div')) : this.options.knobs;
		this.knob_left = knobs[0];
		this.knob_right = knobs[1];
		this.decor = this.options.decor;
		// Maße filtern
		...
		this.range = this.full - this.knob_width; // Knob-Range
                ...
                this.space = Math.round(this.options.minValue/this.faktor);
		...
		this.knob_right.setStyle('position', 'relative').setStyle(this.property, start[1]);
                this.decor.setStyle(this.property, start[0] + this.knob_width);
                this.decor.setStyle(this.offsetSet, start[1] - start[0] - this.knob_width);
	...
	getKnobValues: function()
	{
	  ...
	  values.knob_right = this.options.range[0] + Math.round((this.knob_right.getStyle(this.property).toInt() - this.knob_width) * this.faktor);
	  return values;
	},
  ...
  onDrag: function(draggedElement)
	...
	updateRange: function(left, right){
            this.knob_left.setStyle(this.property, (left - this.options.range[0])/this.faktor);
            this.knob_right.setStyle(this.property, (right - this.options.range[0])/this.faktor + this.knob_width);
            this.updateKnobRange();
        },
	updateKnobRange: function()
	{
	  this.drag_left.options.limit[this.axis[0]] = [
	    0,
	    this.knob_right.getStyle(this.property).toInt() - this.knob_width - this.space
	  ];
	  
	  this.drag_right.options.limit[this.axis[0]] = [
	    this.knob_left.getStyle(this.property).toInt() + this.knob_width + this.space,
	    this.range// - this.knob_width
	  ];
          var left = this.knob_left.getStyle(this.property).toInt()+this.knob_width;
          var right = this.knob_right.getStyle(this.property).toInt();
          this.decor.setStyle(this.property, left);
          this.decor.setStyle(this.offsetSet, right-left);
	}
});